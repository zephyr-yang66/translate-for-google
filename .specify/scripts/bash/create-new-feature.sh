...
# Function to check existing features and return next available number
...
if [ -z "$BRANCH_NUMBER" ]; then
    # Check existing features in specs directory
    HIGHEST=0
    if [ -d "$SPECS_DIR" ]; then
...
if $JSON_MODE; then
    printf '{"FEATURE_NAME":"%s","SPEC_FILE":"%s","FEATURE_NUM":"%s","HAS_GIT":"%s"}\n' "$FEATURE_NAME" "$SPEC_FILE" "$FEATURE_NUM" "$HAS_GIT"
else
